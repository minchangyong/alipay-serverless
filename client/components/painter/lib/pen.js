...
// const QR = require('./qrcode.ts');
// const GD = require('./gradient.ts');
// @ts-ignore
...
export default class Painter {
  ...
  _background() {
    ...
    if (!bg) {
      ...
      GD.doGradient(bg, width, height, this.ctx);
    } else {
      // 背景填充图片
  ...
  _doClip(borderRadius, width, height) {
    if (borderRadius && width && height) {
      ...
      const app = getApp();
      if (!(app.systemInfo &&
        app.systemInfo.version <= '6.6.6' &&
        app.systemInfo.platform === 'ios')) {
        this.ctx.clip();
      }
  ...
  _preProcess(view, notClip) {
    ...
    switch (view.type) {
      ...
      case 'image': {
        ...
        // eslint-disable-next-line no-undef
        const app = getApp();
        const ratio = app.systemInfo.pixelRatio ? app.systemInfo.pixelRatio : 2;
  ...
  _doBackground(view) {
    ...
    if (GD.isGradient(background)) {
      GD.doGradient(background, width, height, this.ctx);
    } else {
  ...
  _drawQRCode(view) {
    ...
    const {
      width,
      height,
    } = this._preProcess(view, false);
    QR.draw(view.content, this.ctx, -width / 2, -height / 2, width, height, view.css.background, view.css.color, '', '');
    this.ctx.restore();
  ...
  _drawAbsRect(view) {
    ...
    if (GD.isGradient(view.css.color)) {
      GD.doGradient(view.css.color, width, height, this.ctx);
    } else {
