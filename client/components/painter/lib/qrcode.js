...
let strinbuf = []; const eccbuf = []; let qrframe = []; const framask = []; const
  rlens = [];
// Control values - width is based on version, last 4 are from table.
let version; let width; let neccblk1; let neccblk2; let datablkw; let
  eccblkwid;
let ecclevel = 2;
  ...
  switch (m) {
    ...
    case 5:
      for (r3y = 0, y = 0; y < width; y++, r3y++) {
        ...
        for (r3x = 0, x = 0; x < width; x++, r3x++) {
          if (r3x == 3) r3x = 0;
          if (!((x & y & 1) + Number(Number(!r3x) | Number(!r3y))) && !ismasked(x, y)) qrframe[x + y * width] ^= 1;
        }
      }
      break;
    case 6:
      for (r3y = 0, y = 0; y < width; y++, r3y++) {
        ...
        for (r3x = 0, x = 0; x < width; x++, r3x++) {
          if (r3x == 3) r3x = 0;
          if (!(((x & y & 1) + Number(r3x && (r3x == r3y))) & 1) && !ismasked(x, y)) qrframe[x + y * width] ^= 1;
        }
      }
      break;
    case 7:
      for (r3y = 0, y = 0; y < width; y++, r3y++) {
        ...
        for (r3x = 0, x = 0; x < width; x++, r3x++) {
          if (r3x == 3) r3x = 0;
          if (!(((r3x && Number(r3x == r3y)) + Number((x + y) & 1)) & 1) && !ismasked(x, y)) qrframe[x + y * width] ^= 1;
        }
      }
      break;
  }
...
const N1 = 3; const N2 = 3; const N3 = 40; const
  N4 = 10;
...
function badcheck() {
  ...
  for (y = 0; y < width; y++) {
    ...
    for (h = b = x = 0; x < width; x++) {
      ...
      if ((b1 = qrframe[x + width * y]) === b) rlens[h] += 1;
  ...
  while (big > width * width) big -= width * width; count += 1;
  thisbad += count * N4;
...
function genframe(instring) {
  let x; let y; let k; let t; let v; let i; let j; let
    m;
  ...
  do {
    ...
    k = datablkw * (neccblk1 + neccblk2) + neccblk2 - 3 + Number(version <= 9);
    if (t <= k) break;
  } while (version < 40);
...
const api = {
   ...
   * @param bg 目前只能设置颜色值
   */
  draw(str, ctx, startX, startY, cavW, cavH, bg, color, $this, ecc) {
    const that = this;
